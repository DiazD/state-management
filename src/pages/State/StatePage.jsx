...
const RenderTree = ({ value }) => {
    // NOTE: we may want to render [] or {} depending on `value` type.
    return <>
        {"{"}
...
const ConstateSlice = ({ heading }) => {
    const [value] = useSubscription(heading.split(" > "), (...x) => x, true);
...
const DataView = () => {
    const selectedSlices = useSubscription(
        ...
        (paths, state) => {
            return paths.map(({ title, value }) => ({ heading: title, value: getIn(value, state) }));
        }
    );
